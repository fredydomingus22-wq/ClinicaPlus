 ...
 */
export async function jobWaTratamentoAtualizacao() {
  const log = logger.child({ job: 'wa-tratamento-atualizacao' });
  ...
  try {
    ...
    for (const plano of planos) {
      ...
      if (!proximaSessao) continue;

      try {
        // Chama API interna para enviar notificação
        await axios.post(`${process.env.API_URL || 'http://localhost:3000'}/api/whatsapp/tratamento-sessao`, {
          planoId: plano.id,
        }, {
          headers: {
 ...
 */
export async function jobWaTratamentoProgresso() {
  const log = logger.child({ job: 'wa-tratamento-progresso' });
