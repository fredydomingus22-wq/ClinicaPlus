    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
      <header className="text-center">
        <Button variant="ghost" className="mb-4 text-neutral-400" onClick={onBack}>
          <ChevronLeft className="w-4 h-4 mr-1" /> Alterar data ou hora
        </Button>
        <h1 className="text-3xl font-black text-neutral-900 tracking-tight">Confirmar Agendamento</h1>
      </header>

      <Card className="rounded-[40px] border-none shadow-xl shadow-neutral-200 overflow-hidden">
        <div className="bg-primary-600 p-8 text-white relative">
          ...
          <div className="relative z-10 flex items-center gap-6">
            ...
            />
            <div>
              <p className="text-sm font-black uppercase tracking-widest text-teal-400">Profissional</p>
              <h3 className="text-2xl font-black italic text-white tracking-tight">Dr. {selectedMedico?.nome}</h3>
              <Badge variant="neutral" className="bg-white/20 text-white border-none mt-1 uppercase font-black text-[10px]">
                {selectedSpecialty}
              </Badge>
            </div>
          </div>
          ...
          <form id="agendamento-form" onSubmit={onSubmit} className="space-y-6 pt-6 border-t border-neutral-100">
            ...
            <div className="flex flex-col gap-3">
              <Button 
                ...
                loading={isPending}
              >
                Confirmar Agendamento — {formatKwanza(5000)}
              </Button>
