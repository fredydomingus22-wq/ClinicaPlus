    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="flex items-center justify-between">
        <Button variant="ghost" className="px-0 hover:bg-transparent text-neutral-400 hover:text-neutral-900" onClick={onBack}>
          <ChevronLeft className="w-5 h-5 mr-1" /> Escolher outro médico
        </Button>
        <div className="text-right">
          <h1 className="text-2xl font-black text-neutral-900">Dr. {selectedMedico?.nome}</h1>
          <p className="text-sm text-primary-600 font-bold">{selectedSpecialty}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <Card className="lg:col-span-5 p-6 rounded-3xl border-neutral-100 shadow-xl shadow-neutral-100/50">
          ...
          <div className="mt-6 p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
            <p className="text-xs text-neutral-600 font-bold uppercase tracking-wider leading-relaxed">
              As consultas têm uma duração média de <span className="text-primary-700">30 minutos</span>. Se não encontrar o horário desejado, contacte a recepção.
            </p>
          </div>
        </Card>

        <Card className="lg:col-span-7 p-6 rounded-3xl border-neutral-100 shadow-xl shadow-neutral-100/50 min-h-[400px]">
          <h3 className="font-black text-lg mb-4 flex items-center gap-2">
              ...
              {slots && slots.length > 0 ? (
                slots.map((slot) => (
                  <button
                    ...
                    className={`
                      p-3 rounded-2xl font-black text-sm transition-all border-2
                  ...
                  >
                    {slot.time}
                    {!slot.available && <span className="block text-[8px] uppercase tracking-tighter mt-0.5">Ocupado</span>}
                  </button>
                ))
              ) : (
                <div className="col-span-full py-12 text-center">
                  <History className="w-8 h-8 text-neutral-300 mx-auto mb-2" />
                  <p className="text-sm text-neutral-600 font-medium">Sem horários disponíveis para este dia.</p>
                </div>
              )}
            </div>
          )}
