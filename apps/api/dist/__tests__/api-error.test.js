...
// ============================================================
(0, vitest_1.describe)('[ADAPTAR: nome do endpoint] — 400 Bad Request', () => {
    ...
    (0, vitest_1.it)('deve lançar ApiError com status 400 quando o payload é inválido', async () => {
        // [ADAPTAR] Simular resposta 400 do servidor
        vitest_1.vi.mocked(fetch).mockResolvedValueOnce(new Response(JSON.stringify({
        ...
        const createUser = vitest_1.vi.fn().mockRejectedValue({ status: 400, name: 'ApiError' });
        // [ADAPTAR] Payload que causa o 400
        const invalidPayload = { email: 'not-an-email', name: '' };
        await (0, vitest_1.expect)(createUser(invalidPayload)).rejects.toMatchObject({
            status: 400,
            name: 'ApiError',
        });
    });
    ...
    vitest_1.it.skip('deve enviar Content-Type: application/json no request', async () => {
        vitest_1.vi.mocked(fetch).mockResolvedValueOnce(new Response(JSON.stringify({ id: 1 }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        }));
        // const { createUser } = await import('../api/users')
        const createUser = vitest_1.vi.fn().mockResolvedValue({ id: 1 });
    ...
    (0, vitest_1.it)('deve enviar exatamente os campos esperados pelo schema', async () => {
        vitest_1.vi.mocked(fetch).mockResolvedValueOnce(new Response(JSON.stringify({ id: 1 }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        }));
        ...
        (0, vitest_1.expect)(body).not.toHaveProperty('undefinedField');
    });
});
// ============================================================
// TEMPLATE B — Reproduzir erro de runtime (TypeError, etc.)
// ============================================================
(0, vitest_1.describe)('[ADAPTAR: nome do componente/hook] — runtime error', () => {
    (0, vitest_1.it)('deve lidar com lista vazia sem lançar TypeError', () => {
    ...
    (0, vitest_1.it)('deve lidar com resposta null da API', async () => {
        vitest_1.vi.spyOn(global, 'fetch').mockResolvedValueOnce(new Response('null', {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        }));
...
// ============================================================
(0, vitest_1.describe)('React Query — não deve retentar em 400', () => {
    (0, vitest_1.it)('retry function deve retornar false para status < 500', () => {
