...
const REQUIRED_BUCKETS = [
    { name: config_1.config.SUPABASE_PUBLIC_BUCKET, public: true },
    { name: config_1.config.SUPABASE_LAUDOS_BUCKET, public: false },
];
/**
 * Configura políticas RLS para os buckets
 */
async function setupRLSPolicies(supabase) {
    const publicBucket = config_1.config.SUPABASE_PUBLIC_BUCKET;
    const privateBucket = config_1.config.SUPABASE_LAUDOS_BUCKET;
...
async function setupStorage() {
    ...
    try {
        ...
        console.log('\n📋 Resumo:');
        console.log(`   - ${config_1.config.SUPABASE_PUBLIC_BUCKET}: público (logos, avatars, docs)`);
        console.log(`   - ${config_1.config.SUPABASE_LAUDOS_BUCKET}: privado (laudos médicos)`);
