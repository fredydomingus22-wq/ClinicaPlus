...
exports.analyticsEstoqueService = void 0;
...
// SERVICE
// ─────────────────────────────────────────────────────────────
exports.analyticsEstoqueService = {
    /**
     * Retorna os KPIs principais do controle de estoque.
     */
    async getKpis(filters) {
        const { clinicaId, dataInicio, dataFim } = filters;
        const inicio = dataInicio ? new Date(dataInicio) : (0, date_fns_1.subDays)(new Date(), 30);
        ...
        const valorEstoqueBatch = await estoque_calculo_service_1.estoqueCalculoService.calcularValorEstoqueBatch(clinicaId, produtoIds);
        ...
        const itensAbaixoMinimo = produtos.filter(p => p.gerenciaEstoque && (estoqueBatch[p.id] || 0) <= p.estoqueMinimo).length;
        // Validades críticas
     ...
     */
    async getTopMovimentados(filters, limite = 20) {
        const { clinicaId, dataInicio, dataFim } = filters;
        const inicio = dataInicio ? new Date(dataInicio) : (0, date_fns_1.subDays)(new Date(), 30);
        const fim = dataFim ? new Date(dataFim) : new Date();
        const movimentacoes = await prisma_1.prisma.movimentacaoEstoque.findMany({
            where: {
                clinicaId,
                criadoEm: { gte: inicio, lte: fim },
                ...(filters.categoriaId ? { produto: { categoriaId: filters.categoriaId } } : {}),
     ...
     */
    async getTendenciaDiaria(filters) {
        const { clinicaId, dataInicio, dataFim } = filters;
        const inicio = dataInicio ? (0, date_fns_1.startOfDay)(new Date(dataInicio)) : (0, date_fns_1.startOfDay)((0, date_fns_1.subDays)(new Date(), 30));
        const fim = dataFim ? (0, date_fns_1.endOfDay)(new Date(dataFim)) : (0, date_fns_1.endOfDay)(new Date());
        const movimentacoes = await prisma_1.prisma.movimentacaoEstoque.findMany({
            where: {
                clinicaId,
                criadoEm: { gte: inicio, lte: fim },
        ...
        // Montar mapa por dia
        const dias = (0, date_fns_1.eachDayOfInterval)({ start: inicio, end: fim });
        const mapaEntradas = {};
        const mapaSaidas = {};
        for (const m of movimentacoes) {
            const key = (0, date_fns_1.format)(m.criadoEm, 'yyyy-MM-dd');
            if (m.tipo === 'ENTRADA') {
     ...
     */
    async getPrevisaoRuptura(clinicaId, diasHistorico = 30) {
        const inicio = (0, date_fns_1.subDays)(new Date(), diasHistorico);
        ...
        // Estoque atual por produto usando service centralizado
        const produtoIds = produtos.map(p => p.id);
        const estoqueAtual = await estoque_calculo_service_1.estoqueCalculoService.calcularEstoqueBatch(clinicaId, produtoIds);
        ...
        return produtos
            ...
            let diasAteRuptura = null;
            let dataEstimadaRuptura = null;
            if (consumoMedioDiario > 0) {
                ...
                if (diasAteRuptura !== null) {
                    ...
                    dataEstimadaRuptura = (0, date_fns_1.format)(data, 'yyyy-MM-dd');
                }
            }
            let criticidade = 'OK';
            if (atual === 0)
                criticidade = 'CRITICA';
            else if (diasAteRuptura !== null && diasAteRuptura <= 7)
                criticidade = 'ALTA';
            else if (diasAteRuptura !== null && diasAteRuptura <= 30)
                criticidade = 'MEDIA';
            return {
                produtoId: p.id,
                nome: p.nome,
            ...
            if (ordem[a.criticidade] !== ordem[b.criticidade])
                return ordem[a.criticidade] - ordem[b.criticidade];
     ...
     */
    async getDistribuicaoCategorias(filters) {
        const { clinicaId, dataInicio, dataFim } = filters;
        const inicio = dataInicio ? new Date(dataInicio) : (0, date_fns_1.subDays)(new Date(), 30);
        const fim = dataFim ? new Date(dataFim) : new Date();
        const [categorias, lotes, movimentacoes] = await Promise.all([
            prisma_1.prisma.categoriaProduto.findMany({
        ...
        const produtosParaMov = await prisma_1.prisma.produto.findMany({
