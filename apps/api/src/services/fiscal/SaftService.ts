...
export class SaftService {
  ...
  public async generateXML(options: SaftExportOptions): Promise<string> {
    ...
    header.ele('ProductCompanyTaxID').txt(process.env.AGT_PRODUCT_COMPANY_TAX_ID || '');
    header.ele('SoftwareValidationNumber').txt(process.env.AGT_VALIDATION_NUMBER || '0/AGT/2020');
    header.ele('ProductID').txt(process.env.AGT_PRODUCT_ID || 'ClinicaPlus/ClinicaPlus-Software-Lda');
    header.ele('ProductVersion').txt(process.env.AGT_PRODUCT_VERSION || '1.0.0');
