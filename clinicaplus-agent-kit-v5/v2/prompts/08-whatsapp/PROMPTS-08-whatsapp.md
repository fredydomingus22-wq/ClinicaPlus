...
## Passo 2 — Clientes HTTP (TDD)
- [x] evolutionApi.ts (RED -> GREEN -> REFACTOR)
- [x] n8nApi.ts (RED -> GREEN -> REFACTOR)
- [x] Templates n8n (marcacao, lembretes, confirmacao, boas-vindas)
...
- [x] RED: Criar testes para `wa-lembrete.job` e `wa-expirar-conversas.job` ✅
- [x] GREEN: Implementar lógica de busca e enfileiramento ✅
- [x] GREEN: Adicionar ao `scheduler.service.ts` ✅
- [x] REFACTOR: Melhorar formatação de mensagens e logs ✅

Implementado no scheduler:
```typescript
cron.schedule('0 7 * * *', async () => { ... }, { timezone: 'Africa/Luanda' }); // 24h
cron.schedule('0 * * * *', async () => { ... }, { timezone: 'Africa/Luanda' }); // Expiração
cron.schedule('*/30 * * * *', async () => { ... }, { timezone: 'Africa/Luanda' }); // 2h
```
