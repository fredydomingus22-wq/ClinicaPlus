...
function detectMonorepoRoot(): string {
  ...
  while (true) {
    ...
    if (parent === dir) break  // chegou na raiz do sistema (C:\ ou /)
    dir = parent
  }
  return process.cwd()
}
...
function runVitestInApp(appPath: string, filter?: string): Promise<boolean> {
  return new Promise(resolve => {
    ...
    const cmdArgs = pm === 'npm' ? [...vitestArgs] : vitestArgs
    ...
    const child = spawn(cmd, cmdArgs, {
      ...
      shell: true,  // necessário no Windows para resolver .cmd do PATH
    })
    ...
    const walk = (dir: string) => {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') {
          ...
          !entry.name.includes('fetch-wrapper') &&  // skip — é a própria implementação
          !entry.name.includes('queryClient')        // skip — infra, não chamadas de negócio
        ) {
